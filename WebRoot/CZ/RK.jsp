...
<html>
  <head>
    ...
    <title>My JSP 'RK.jsp' starting page</title>
  ...
  <body>
  <div class="form-control" style="overflow: hidden;padding: 1px;">入库管理</div>
  <div class="form-control">入库操作</div>
  <form action="RK" method="post" class="form-inline" >
  ...
  <input name="OGCP" class="form-control" placeholder="单价"></input>
  <select class="selectpicker show-tick form-control" data-live-search="true" name="OGCG">
  <%for(int i=0;i<gys.size();i++) {%>
  <option><%=gys.get(i).getGYN() %></option>
  <%} %>
  </select>
  <Button class="btn btn-default" type="submit">入库</Button>
  </form>
  <div class="form-control">入库记录</div>
  ...
  <td class="info">入库产品</td>
  <td class="success">单价</td>
  <td class="success">供应商</td>
  <td class="active">入库数量</td>
  <td class="active">入库时间</td>
  </tr>
  <%ArrayList<GCT> gct=new ArrayList<GCT>(); gct=cz.getRK(); %>
